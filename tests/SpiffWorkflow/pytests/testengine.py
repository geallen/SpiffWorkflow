...
class BpmnTestEngine(object):
    ...
    def do_next_named_step(self, step_name, with_save_load=False, set_attribs=None, choice=None,
        ...
        def is_match(task):
            if not (task.task_spec.name == step_name_path[-1] or task.task_spec.description == step_name_path[-1]):
                return False
    ...
    @staticmethod
    def _do_single_step(step_name, tasks, set_attribs=None, choice=None, only_one_instance=True):

        if only_one_instance:
            assert len(tasks) == 1, 'Did not find one task for \'%s\' (got %d)' % (step_name, len(tasks))
        else:
            assert len(tasks) > 0, 'Did not find any tasks for \'%s\'' % step_name

        assert True == tasks[0].task_spec.name == step_name or tasks[0].task_spec.description == step_name, \
            'Expected step %s, got %s (%s)' % (
                step_name, tasks[0].task_spec.description, tasks[0].task_spec.name)
        if not set_attribs:
            set_attribs = {}
